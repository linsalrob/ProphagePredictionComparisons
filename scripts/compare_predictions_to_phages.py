...
import sys
import argparse
from ProphagePredictionsLib import genbank_seqio
...
def predicted_genbank(predf, verbose=False):
    ...
    if verbose:
        sys.stderr.write(f"Found {len(predicted)} predicted prophage features\n")
...
def compare_real_predicted(phage: dict, nonphage: dict, predicted: dict, print_fp: bool, print_fn: bool, verbose: bool):
    ...
    if verbose:
        sys.stderr.write(f"Comparing real and predicted\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--testfile', required=True,
                        help='test file that has phage proteins marked with the is_phage qualifier')
    parser.add_argument('-p', '--predictfile',
                        help='predictions genbank file that has each prophage as a sequence entry')
    ...
    if args.predictfile:
        ...
        sys.stderr.write("FATAL: Please provide either a predictions genbank or tsv file\n")
